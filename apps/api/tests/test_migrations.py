...
@pytest.mark.asyncio
async def test_candidate_coordinate_checked_at_migration_is_recorded() -> None:
    await run_migrations()
        ...
        count = (
            await session.execute(
                text(
                    ...
                    SELECT count(*)
                    FROM schema_migrations
                    WHERE version = '0005_candidate_coordinate_checked_at'
                    """
                )
            )
        ).scalar_one()
        ...
        column_count = (
            await session.execute(
                text(
                    ...
                    FROM information_schema.columns
                    WHERE table_name = 'place_candidates'
                      AND column_name = 'coordinate_checked_at'
                    """
                )
            )
        ).scalar_one()
