...
def test_nakhon_ratchasima_khao_yai_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/nakhon_ratchasima_khao_yai_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 9
    assert {candidate.province for candidate in candidates} == {"นครราชสีมา"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 2
...
def test_nakhon_ratchasima_khao_yai_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/nakhon_ratchasima_khao_yai_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_prachuap_hua_hin_expansion_seed_has_balanced_coverage() -> None:
    path = Path("../../database/seeds/prachuap_hua_hin_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 11
    assert {candidate.province for candidate in candidates} == {"ประจวบคีรีขันธ์"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 5
...
def test_prachuap_hua_hin_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/prachuap_hua_hin_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_phetchaburi_cha_am_expansion_seed_has_balanced_coverage() -> None:
    path = Path("../../database/seeds/phetchaburi_cha_am_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 11
    assert {candidate.province for candidate in candidates} == {"เพชรบุรี"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 5
...
def test_phetchaburi_cha_am_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/phetchaburi_cha_am_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_ratchaburi_expansion_seed_has_balanced_coverage() -> None:
    path = Path("../../database/seeds/ratchaburi_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 11
    assert {candidate.province for candidate in candidates} == {"ราชบุรี"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
...
def test_ratchaburi_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/ratchaburi_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_nakhon_pathom_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/nakhon_pathom_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 7
    assert {candidate.province for candidate in candidates} == {"นครปฐม"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 1
    assert sum(candidate.place_type == "RESTAURANT" for candidate in candidates) == 5
    assert sum(candidate.place_type == "ACCOMMODATION" for candidate in candidates) == 1


def test_nakhon_pathom_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/nakhon_pathom_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_samut_sakhon_mahachai_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/samut_sakhon_mahachai_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 7
    assert {candidate.province for candidate in candidates} == {"สมุทรสาคร"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 1
    assert sum(candidate.place_type == "RESTAURANT" for candidate in candidates) == 6
    assert sum(candidate.place_type == "ACCOMMODATION" for candidate in candidates) == 0


def test_samut_sakhon_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/samut_sakhon_mahachai_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_samut_songkhram_amphawa_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/samut_songkhram_amphawa_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 8
    assert {candidate.province for candidate in candidates} == {"สมุทรสงคราม"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 1
...
def test_samut_songkhram_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/samut_songkhram_amphawa_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_samut_prakan_expansion_seed_has_balanced_coverage() -> None:
    path = Path("../../database/seeds/samut_prakan_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 11
    assert {candidate.province for candidate in candidates} == {"สมุทรปราการ"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 5
...
def test_samut_prakan_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/samut_prakan_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_nonthaburi_expansion_seed_has_balanced_coverage() -> None:
    path = Path("../../database/seeds/nonthaburi_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 11
    assert {candidate.province for candidate in candidates} == {"นนทบุรี"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 5
...
def test_nonthaburi_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/nonthaburi_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_pathum_thani_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/pathum_thani_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 8
    assert {candidate.province for candidate in candidates} == {"ปทุมธานี"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 5
...
def test_pathum_thani_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/pathum_thani_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_nakhon_nayok_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/nakhon_nayok_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 8
    assert {candidate.province for candidate in candidates} == {"นครนายก"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 5
...
def test_nakhon_nayok_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/nakhon_nayok_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_saraburi_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/saraburi_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 9
    assert {candidate.province for candidate in candidates} == {"สระบุรี"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 5
...
def test_saraburi_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/saraburi_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_lopburi_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/lopburi_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 7
    assert {candidate.province for candidate in candidates} == {"ลพบุรี"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 5
...
def test_lopburi_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/lopburi_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_ang_thong_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/ang_thong_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 6
    assert {candidate.province for candidate in candidates} == {"อ่างทอง"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 3
...
def test_ang_thong_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/ang_thong_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_chai_nat_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/chai_nat_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 4
    assert {candidate.province for candidate in candidates} == {"ชัยนาท"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 1
...
def test_chai_nat_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/chai_nat_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_uthai_thani_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/uthai_thani_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 6
    assert {candidate.province for candidate in candidates} == {"อุทัยธานี"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 2
...
def test_uthai_thani_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/uthai_thani_expansion_review_queue.csv")
    candidates = load_candidates(path)
...
def test_suphan_buri_expansion_uses_conservative_batch_size() -> None:
    path = Path("../../database/seeds/suphan_buri_expansion_review_queue.csv")
    candidates = load_candidates(path)

    assert len(candidates) == 6
    assert {candidate.province for candidate in candidates} == {"สุพรรณบุรี"}
    assert all(candidate.review_state == "DISCOVERED" for candidate in candidates)
    assert all(candidate.latitude is None for candidate in candidates)
    assert all(candidate.longitude is None for candidate in candidates)
    assert sum(candidate.place_type == "MOSQUE" for candidate in candidates) == 1
...
def test_suphan_buri_commercial_candidates_do_not_overclaim_trust() -> None:
    path = Path("../../database/seeds/suphan_buri_expansion_review_queue.csv")
    candidates = load_candidates(path)
