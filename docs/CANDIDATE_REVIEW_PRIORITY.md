...
For the current 7 Google-resolvable pilot candidates, review the **5**
no-hold items first so the queue shrinks quickly, then handle the two
Google-resolvable candidates with explicit holds separately.
